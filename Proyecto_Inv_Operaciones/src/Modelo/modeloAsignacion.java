...
public class modeloAsignacion {
    ...
    public static String resolver(ProblemaAsignacion p) {
        if (p == null || p.costos == null || p.costos.length == 0 || p.costos[0].length == 0) {
            return "Error: datos de entrada vacíos o nulos.";
        }
        ...
        for (int i = 1; i < m; i++) {
            if (p.costos[i].length != n) {
                return "Error: la matriz de costos no es rectangular.";
            }
        }
        ...
        for (int i = 0; i < m; i++) {
            ...
            sb.append(fila).append("-").append(col).append(" = ").append(cost);
        }

        sb.append("\nCosto total mínimo = ").append(total);
        return sb.toString();
    }
    ...
    // ======= Clase interna del problema =======
    public static class ProblemaAsignacion {
        public final double[][] costos;
        public final String[] filas;
        public final String[] columnas;

        public ProblemaAsignacion(double[][] costos, String[] filas, String[] columnas) {
            this.costos = costos;
            this.filas = filas;
